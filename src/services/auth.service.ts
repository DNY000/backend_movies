...
export class AuthService {
  ...
  async register(userData: {
    ...
    // Disallow registering admin accounts via public API
    if (userData.role && userData.role !== UserRole.USER) {
      throw new Error('Registration with admin role is not allowed')
    }
  ...
  async loginUser(email: string, password: string): Promise<{ user: User; accessToken: string; refreshToken: string }> {
    return this.loginWithRequiredRole(email, password, UserRole.USER)
  }
  ...
  async loginAdmin(
    ...
    return this.loginWithRequiredRole(email, password, UserRole.ADMIN)
  }
  ...
  private async loginWithRequiredRole(
    ...
    const base = await this.login(email, password)
    if (base.user.role !== requiredRole) {
